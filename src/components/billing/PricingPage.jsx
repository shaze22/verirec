...
const COUNSELOR_FEATURES = [
  ...
  'Borang tempahan QR — klien book sendiri',
  'Fail klien digital + sejarah sesi',
  'Nota Sesi Kes format SOP (PDF)',
  'Penilaian MBTI + RIASEC dalam sesi',
  'Laporan AI + eksport PDF',
  'Jadual & Temujanji + reminder',
  'Plan Tindakan & Rujukan Klinikal',
...
export function PricingPage() {
  ...
  const subscribe = async (plan) => {
    if (plan === 'free') return;
    if (plan === 'enterprise') {
      window.location.href = 'mailto:hello@verirec.app?subject=Pertanyaan Pelan Institusi VeriRec';
      return;
    }
    setLoading(plan);
    try {
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 mt-2">Satu harga. Tanpa kejutan. Top-up bila perlu.</p>
      </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
            ...
            <div className={`rounded-2xl border-2 p-6 flex flex-col ${
              ...
              <ul className="space-y-2 flex-1 mb-6">
                ...
                {['AI analisis realtime', 'Panduan PEACE Model', 'Top-up sesi'].map(f => (
            ...
            {/* Professional */}
            <div className={`relative rounded-2xl border-2 p-6 flex flex-col shadow-lg ${
              isCurrent('starter') ? 'border-blue-400 bg-blue-50' : 'border-blue-600 bg-white'
            }`}>
              {!isCurrent('starter') && (
              ...
              {isCurrent('starter') && (
                <p className="text-xs text-blue-700 font-medium text-center mb-3">
                  Baki: {sessionBalance} sesi{extraSessions > 0 ? ` + ${extraSessions} top-up` : ''}
                </p>
              )}
              <Button
                variant="primary"
                className="w-full"
                disabled={isCurrent('starter')}
              ...
              {CONFIG.topups.map(t => (
                <div key={t.key} className="flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-xs text-gray-400 ml-2">RM{t.perSession}/sesi</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-gray-900">RM{t.price}</span>
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={subscription?.plan !== 'starter' || loading === t.key}
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="grid lg:grid-cols-2 gap-8 items-start">
            ...
            <div className={`relative rounded-2xl border-2 p-6 flex flex-col ${
              ...
              {isCurrent('counselor') && (
                <p className="text-xs text-center text-emerald-700 font-medium mb-3">
                  Baki: {sessionBalance} sesi{extraSessions > 0 ? ` + ${extraSessions} top-up` : ''}
                </p>
              )}
              <Button
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-gray-500">Tidak luput — dibawa ke bulan berikutnya.</p>
              </div>

              {CONFIG.topups.map(topup => (
                <div key={topup.key} className="rounded-xl border bg-white p-4 flex items-center justify-between gap-4">
                  <div>
                    <p className="font-semibold text-gray-900">{topup.label}</p>
                    <p className="text-xs text-gray-400 mt-0.5">RM{topup.perSession}/sesi</p>
                  </div>
                  <div className="flex items-center gap-3">
              ...
              <div className="bg-emerald-50 rounded-xl border border-emerald-100 p-4">
                <p className="text-sm font-semibold text-emerald-800 mb-1">Kos setiap klien yang jelas</p>
                <p className="text-sm text-emerald-700">RM10–13 per sesi. Mudah kira bila nak charge klien anda.</p>
              </div>
            </div>
          </div>
        </div>
