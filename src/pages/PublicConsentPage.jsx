...
const CONSENT_ITEMS = [
  'I understand that this session will be audio-recorded and transcribed using AI technology.',
  'I agree that the recording and transcript will be used solely for the stated professional purpose.',
  'I understand that my personal information is protected and handled in accordance with applicable privacy laws.',
  'I agree to participate in this session voluntarily and understand I may stop at any time.',
];
...
export default function PublicConsentPage() {
  ...
  useEffect(() => {
    ...
    supabaseAnon
      ...
      .select('id, subject_name, purpose, interviewer_name, consent_given, expires_at')
      .eq('token', token)
      .single()
      .then(({ data, error }) => {
  ...
  if (status === 'expired') return <StatusPage icon="⏰" title="Link Expired" desc="This consent link has expired. Please contact the officer for a new link." />;
  ...
  if (status === 'error') return <StatusPage icon="⚠️" title="Something went wrong" desc="Please try again or contact the officer." />;
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black text-gray-900">Session Consent Form</h1>
          <p className="text-gray-500 mt-2 text-sm">Please read carefully and provide your consent below.</p>
        </div>

        <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
          <div className="space-y-3 mb-6">
            {record.subject_name && <InfoRow label="Subject" value={record.subject_name} />}
            {record.interviewer_name && <InfoRow label="Officer" value={record.interviewer_name} />}
        ...
        <button
          ...
          className="w-full py-4 bg-blue-600 text-white font-bold rounded-xl disabled:opacity-40 disabled:cursor-not-allowed hover:bg-blue-700 transition-colors text-sm"
        >
