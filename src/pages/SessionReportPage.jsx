...
export default function SessionReportPage() {
  ...
  const handleSendToSubject = async () => {
    setAckSending(true);
    try {
      ...
      const url = `${window.location.origin}/statement/${data.token}`;
      const text = `Dear ${session?.subject_name || 'Sir/Madam'},\n\nPlease review and acknowledge your statement from the session "${session?.title}":\n\n${url}\n\nThis link is valid for 30 days.\n\n— VeriRec`;
      window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
      toast.success('Statement link ready — select recipient in WhatsApp.');
    } catch {
      toast.error('Failed to create statement link.');
    } finally {
      setAckSending(false);
    }
  };
